...
void CSpatialPartition::PrintSelf() const
{
 ...
 bool CSpatialPartition::IsVisible(Vector3 theCameraPos, Vector3 theCameraDir, const int xIndex, const int zIndex)
 {
	 float xDistance = (xGridSize * xIndex + (xGridSize >> 1) - (xSize >> 1)) - theCameraPos.x;
	 float zDistance = (zGridSize * zIndex + (zGridSize >> 1) - (zSize >> 1)) - theCameraPos.z;
	 Vector3 gridCentre(xDistance, 0, zDistance);
	 if (theCameraDir.Dot(gridCentre) < 0)
		 return false;
	 return true;
 }
